...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"Retrieval warmup notice: {e}")

    # Also warmup active vLLM generator if running
    try:
        model_name = config.get_active_vllm_model(config.GEN_BASE_URL)
        requests.post(
            f"{config.GEN_BASE_URL}/chat/completions",
            json={
                "model": model_name,
...
async def generate_grounded_answer_async(
    ...
    Safely manages context window to prevent 400 Bad Request on tight limits (e.g. 4096 max-model-len).
    """
    if model_name is None:
        model_name = config.get_active_vllm_model(base_url)
    import httpx
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # If 400 Bad Request due to context length ceiling, adaptively reduce tokens & chunks
            if "400" in str(e) and len(active_chunks) > 3:
                active_chunks = active_chunks[:max(3, len(active_chunks) - 2)]
                max_gen_tokens = 256
